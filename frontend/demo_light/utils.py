...
class QdrantVectorStoreManager:
    ...
    def _check_create_collection(
        client: QdrantClient, collection_name: str, model: OpenAIEmbeddings
    ):
        """Always delete and recreate the collection."""
        if client is None:
    ...
    def _init_online_vector_db(
        url: str, api_key: str, collection_name: str, model: OpenAIEmbeddings
    ):
    ...
    def _init_offline_vector_db(
        vector_store_path: str, collection_name: str, model: OpenAIEmbeddings
    ):
    ...
    @staticmethod
    def create_or_update_vector_store_from_docs(
        collection_name: str,
        vector_db_mode: str,
        ...
        """
        Creates or updates the vector store using a list of Document objects.

        Args:
            collection_name: Name of the Qdrant collection.
            vector_db_mode (str): Mode of the Qdrant vector store ('online' or 'offline').
            documents (List[Document]): List of Document objects to index.
            chunk_size: Size of each text chunk for splitting documents.
            chunk_overlap: Overlap between text chunks.
            batch_size (int): Batch size for adding documents to the collection.
            vector_store_path (str): Path to the vector store (for offline mode).
            url (str): URL of the Qdrant server (for online mode).
            qdrant_api_key: API key for the Qdrant server (for online mode).
            embedding_model: Name of the OpenAI embedding model.
            device: Device to run the embeddings model on ('cpu', 'cuda', 'mps').
        """
        ...
        split_documents = text_splitter.split_documents(documents)
        logging.info(f"Created {len(split_documents)} chunks")
            ...
            doc.metadata.update(
                {
                    ...
                    "url": doc.metadata.get("source_url", doc.metadata.get("url", "")),
                    "description": doc.metadata.get(
                        "source_description", doc.metadata.get("description", "None")
                    ),
                    "chunk_index": idx,
                }
            )
