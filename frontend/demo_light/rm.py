...
class VectorAndBingSearch(dspy.Retrieve):
    def __init__(
        self,
        bing_search_api_key=None,
        is_valid_source: Callable = None,
    ...
    def get_usage_and_reset(self):
        ...
        return {"VectorAndBingSearch": usage}
    ...
    def forward(
        ...
        """Search with Bing and in your data for self.k top passages for query or queries

        Args:
            query_or_queries (Union[str, List[str]]): The query or queries to search for.
            exclude_urls (List[str]): A list of urls to exclude from the Bing search results.
                ...
                for i in range(0, 5):
                    try:
                        related_docs = self.qdrant.similarity_search_with_score(
                            query, k=self.k, filter=self.filter_criteria
                        )
                        if related_docs:
                            print(
                                f"Documents after performing similarity search with score: {len(related_docs)!r}"
                            )
                            break
                    except:
...
class VectorAndYouSearch(dspy.Retrieve):
    def __init__(
        self,
        you_search_api_key=None,
        is_valid_source: Callable = None,
        collection_name: str = None,
        uploaded_file_id: str = None,
        embedding_model: str = "BAAI/bge-m3",
        ...
        **kwargs,
        ...
        """
        Params:
            you_search_api_key: Your You.com API key.
            is_valid_source: A function that takes a URL and returns a boolean indicating if the
                source is valid. Defaults to always True.
            collection_name: The name of your Qdrant collection.
            uploaded_file_id: Optional ID if you're restricting vector searches to a subset of docs.
            embedding_model: Name of the Hugging Face embedding model used for the vector store.
            device: Which device to run the model on ("mps", "cuda", or "cpu").
            k: Number of top results to retrieve from each search (You + Vector).
        """
        super().__init__(k=k)

        # -- Replace BingSearch with You.com retriever --
        self.you = YouRM(
            ydc_api_key=you_search_api_key,
            k=k,
        ...
        # Build a Qdrant Filter so only docs matching file_id are retrieved
        self.filter_criteria = Filter(
            must=[
                FieldCondition(
                    key="metadata.file_id",
                    match=MatchValue(value=self.uploaded_file_id),
                )
            ]
        )

        # The rest is the same vector logic as in VectorAndBingSearch:
        if not collection_name:
            raise ValueError("Please provide a collection name.")
        model_kwargs = {
    ...
    def init_online_vector_db(self, url: str, api_key: str):
        if api_key is None:
            if not os.getenv("QDRANT_API_KEY"):
                raise ValueError("Please provide a Qdrant API key.")
            api_key = os.getenv("QDRANT_API_KEY")
        if url is None:
            raise ValueError("Please provide the Qdrant server URL.")

        try:
            self.client = QdrantClient(url=url, api_key=api_key)
            self._check_collection()
        except Exception as e:
            raise ValueError(f"Error connecting to Qdrant server: {e}")

    def init_offline_vector_db(self, vector_store_path: str):
        if vector_store_path is None:
            raise ValueError("Please provide a local Qdrant database path.")

        try:
            self.client = QdrantClient(path=vector_store_path)
            self._check_collection()
        except Exception as e:
            raise ValueError(f"Error loading the local Qdrant database: {e}")

    def get_usage_and_reset(self):
        usage = self.usage
        self.usage = 0
        return {"VectorAndYouSearch": usage}
    ...
    def forward(
        ...
        """
        Search with You.com AND your vector DB for the top k passages.

        Returns:
            A list of dicts with 'description', 'snippets', 'title', 'url'.
        """
        queries = (
            [query_or_queries]
            if isinstance(query_or_queries, str)
        ...
        for query in queries:
            try:
                # 1) You.com results
                you_results = self.you.forward(query, exclude_urls=exclude_urls)
                collected_results.extend(you_results)
            except Exception as e:
                logging.error(f"Error with You.com search for query '{query}': {e}")

            try:
                # 2) Vector retrieval results
                # Attempt the similarity search with a small retry loop
                related_docs = []
                for _ in range(5):
                    try:
                        related_docs = self.qdrant.similarity_search_with_score(
                ...
                for doc, _score in related_docs:
                    collected_results.append(
                        {
                ...
                logging.error(f"Error with vector search for query '{query}': {e}")
